...
def get_stats():
    ...
    return jsonify({
        ...
        "recent_logs": recent_activity[-10:],
...
app = Flask(__name__)
CORS(app)  # <-- 2. Enable CORS globally for all API routes

# In-memory log for recent scans
recent_activity = []

# ==========================================
# PURE API ROUTES (No HTML rendering)
# ==========================================
...
def process_image():
    try:
        ...
        # Log the activity if it's a match
        if judgment['match']:
            log_entry = {
                "name": judgment['name'],
                "time": time.strftime('%H:%M:%S'),
...
if __name__ == "__main__":
    # Your backend AI engine is now running exclusively on Port 5000
    app.run(host='0.0.0.0', port=5000, debug=True)